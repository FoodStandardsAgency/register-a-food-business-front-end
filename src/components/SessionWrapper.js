const SessionWrapper = (Page) => {
    ...
    ************************************************************************************/
        const initialProps = {
            ...
            switches:
                req && req.session && req.session.switches
            ...
            submissionError:
                req && req.session && req.session.submissionError
                    ...
                    : [],
            emailFbo:
                req && req.session && req.session.emailFbo
                    ? req.session.emailFbo
                    : {},
            lcConfig:
                req && req.session && req.session.lcConfig
                    ? req.session.lcConfig
                    : {},
            addressLookups:
                req && req.session && req.session.addressLookups
                    ...
                    : {},
            council:
                req && req.session && req.session.council
                    ? req.session.council
                    : "",
            isBrowserSupported:
                req && req.session && req.session.isBrowserSupported
                    ...
                    : false,
            browser:
                req && req.session && req.session.browser
                    ? req.session.browser
                    : "",
            browserVersion:
            ...
            isBrowserVersionVerified:
                req && req.session && req.session.isBrowserVersionVerified
                    ...
                    : false,
            country:
                req && req.session && req.session.country
                    ? req.session.country
                    : "",
            lcName:
                req && req.session && req.session.lcName
                    ? req.session.lcName
                    : ""
        };
