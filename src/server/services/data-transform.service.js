    ...
    try {
        ...
        if (data.operator_address_selected) {
            if (data.operator_address_line_1) {
                ...
                const operatorAddressLookupData =
                    addressLookups.operator_postcode_find[
                        data.operator_address_selected
                    ];
                ...
                data.operator_uprn = trimUprn(
                    operatorAddressLookupData["uprn"]
                );
        ...
        if (data.establishment_address_selected) {
            if (data.establishment_address_line_1) {
                ...
                data.establishment_street =
                    establishmentAddressLookupData["street"];

                data.establishment_town =
                    establishmentAddressLookupData["posttown"];
...
const transformAnswersForSubmit = (
    ...
    operator_keys.forEach((key) => {
        if (submitData[key] !== undefined) {
            submitObject.registration.establishment.operator[key] =
                submitData[key];
        }
    });
    ...
    premise_keys.forEach((key) => {
        if (submitData[key] !== undefined) {
            submitObject.registration.establishment.premise[key] =
                submitData[key];
        }
    });
