...
describe("Establishment-Address-Type", () => {
  ...
  it("renders 3 radio buttons with correct error props and default values", async () => {
    ...
    const $ = renderPage(
      ...
      `language: 'en', validatorErrors:${testValidatorErrors}`
    );

    const $establishmentAddressTypeRadio = $("Radio");
    expect(
      $establishmentAddressTypeRadio._root[0].children[1].children.length
    ).toBe(3);
  });
  ...
  it("renders the correct error", async () => {
    ...
    const $establishmentAddressTypeRadio = $(".govuk-error-summary__list a");
    expect($establishmentAddressTypeRadio.length()).toBe(1);
    expect($establishmentAddressTypeRadio.get(0).text()).toBe("test error");
  });
